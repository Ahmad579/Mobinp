//
//  NotaryReviewCell.swift
//  Mobinp
//
//  Created by Ahmed Durrani on 14/03/2018.
...
protocol UploadCertificate {
    func selectCertificateToUploa(checkCell : NotaryReviewCell , indexPath : IndexPath)
    
}


class NotaryReviewCell: UITableViewCell {
    
    var delegate: UploadCertificate?
    var index: IndexPath?
//    var responseObj: UserResponse?
    ...
    @IBAction func btnUploadCertificate(_ sender: UIButton) {
        sender.isSelected = !sender.isSelected
        self.delegate?.selectCertificateToUploa(checkCell : self  , indexPath : index!)
...
extension NotaryReviewCell : UICollectionViewDelegate ,UICollectionViewDelegateFlowLayout, UICollectionViewDataSource
{
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return (self.responseObj?.certificateList?.count)!
    }
