...
class WebServiceManager: NSObject {
    ...
    class func post<T: AnyObject>(params: Dictionary<String, AnyObject>, serviceName: String, serviceType: String, modelType: T.Type, success: @escaping ( _ servicResponse: AnyObject) -> Void, fail: @escaping ( _ error: NSError) -> Void, showHUD: Bool)  where T: Mappable {
        ...
        Alamofire.upload(multipartFormData:{ multipartFormData in
            if profileImage != nil {
                if let imageData = UIImageJPEGRepresentation(profileImage!, 0.5) {
                    multipartFormData.append(imageData, withName:imageParam, fileName:"profile_image", mimeType: "image/png")
                }
            }
                // import parameters
            for (key, value) in params {
